...
function set_last(base, temp, error)
{
   //改变四级级联下拉菜单的选项供选择
	...
	if (base.value == '词语错误'){
		...
		//alert(temp.text + ":" + temp.value);
		error.options[0].text = document.getElementById('temperror').value; 
		error.options[0].value = document.getElementById('temperror').value; 
	}
}